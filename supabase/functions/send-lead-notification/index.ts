...
interface NotificationRequest {
  ...
  assigned_at_timestamp?: string | null;
...
function generateSlaWarningEmailHtml(lead: Lead, adminName: string, leadDetailUrl: string, assignedAgentName: string, timeSinceAssignment: number, slaMinutes: number, assignedAgentEmail?: string | null, assignedAtTimestamp?: string | null): string {
  const normalizedLead = normalizeLead(lead);
  const flag = getLanguageFlag(normalizedLead.language!);
  const segmentColor = getSegmentColor(normalizedLead.lead_segment!);
  
  // Format claimed-at timestamp
  let claimedAtDisplay = 'Unknown';
  try {
    if (assignedAtTimestamp) {
      const d = new Date(assignedAtTimestamp);
      claimedAtDisplay = d.toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Europe/Madrid' });
    }
  } catch { /* fallback */ }
  
  // Agent email row
  const agentEmailRow = assignedAgentEmail ? `<p style="margin: 4px 0 0; color: #92400E; font-size: 13px;"><strong>Agent Email:</strong> <a href="mailto:${assignedAgentEmail}" style="color: #B45309;">${assignedAgentEmail}</a></p>` : '';
  const claimedAtRow = `<p style="margin: 4px 0 0; color: #92400E; font-size: 13px;"><strong>Claimed At:</strong> ${claimedAtDisplay} (Madrid time)</p>`;
...
serve(async (req) => {
  ...
  try {
    ...
    const { lead, agents, claimWindowMinutes, notification_type, lead_priority, assigned_agent_name, assigned_agent_email, assigned_at_timestamp, time_since_assignment_minutes, triggered_by, trigger_reason, scheduled_release_at, form_name, form_data, utm_source, utm_campaign }: NotificationRequest = await req.json();
    ...
    for (let i = 0; i < shuffledAgents.length; i++) {
      ...
      try {
        ...
        if (useFormSubmissionAlertTemplate) {
          ...
          emailSubject = `CRM_ADMIN_SLA_WARNING_${slaLangCode} | Lead claimed but not worked (${claimWindowMinutes}-min SLA breach)`;
        } else if (useAdminUnclaimedTemplate) {
