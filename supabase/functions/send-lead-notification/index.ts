...
serve(async (req) => {
  ...
  try {
    ...
    const results: Array<{ agent: string; emailSuccess: boolean; emailType?: 'standard' | 'urgent' | 'admin_unclaimed'; error?: string }> = [];
    ...
    for (const agent of agents) {
      ...
      const emailType = useAdminUnclaimedTemplate ? 'admin_unclaimed' : (useUrgentTemplate ? 'urgent' : 'standard');
    ...
    console.log(`[send-lead-notification] Summary: ${emailSuccessCount}/${agents.length} emails sent (${urgentEmailCount} urgent)`);
