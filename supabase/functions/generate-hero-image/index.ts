...
serve(async (req) => {
  ...
  try {
    ...
    // Mobile prompt - couple in UPPER THIRD (so text overlay doesn't cover them)
    const mobilePrompt = `A photorealistic, high-end lifestyle photograph of an attractive couple in their late 50s relaxing at a luxury modern Mediterranean villa in Costa del Sol. The couple is positioned in the UPPER THIRD of the frame, sitting intimately together on a plush beige outdoor lounge sofa on an elevated terrace or balcony. They are holding champagne glasses and leaning into each other warmly. A champagne bottle in an ice bucket sits nearby with fruit. The villa features modern architecture with natural stone walls. The LOWER HALF of the image shows the infinity pool, Mediterranean landscaping, and sea views - this area should have NO PEOPLE. WARM GOLDEN HOUR lighting - soft, luminous, and well-exposed. Beige, cream, and taupe color palette. Shot in the style of Architectural Digest, 8k resolution, portrait composition with subjects at TOP, cinematic depth of field. CRITICAL: Couple must be in the UPPER THIRD of the frame. The CENTER and BOTTOM of the image must be clear of people - show pool, landscaping, and sea views there.`;
