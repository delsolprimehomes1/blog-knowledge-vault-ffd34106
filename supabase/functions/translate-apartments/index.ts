...
Do NOT translate Spanish place names (like Marbella, Estepona, Costa del Sol, etc.) - keep them as-is.
Do NOT translate brand names like "Del Sol Prime Homes".
For property types, translate naturally: apartment→local equivalent, villa→villa, townhouse→local equivalent.
...
serve(async (req) => {
  ...
  try {
    ...
    if (pageErr || !pageContent) {
      throw new Error(`Failed to fetch English page content: ${pageErr?.message}`);
    }
    ...
    if (propErr || !properties?.length) {
      throw new Error(`Failed to fetch English properties: ${propErr?.message}`);
    }
    ...
    for (const lang of targetLangs) {
      try {
        ...
        // 5. Upsert translated page content
        const { error: upsertErr } = await supabase
          ...
          .upsert({
            ...
            video_thumbnail_url: pageContent.video_thumbnail_url,
            video_enabled: pageContent.video_enabled,
            reviews_enabled: pageContent.reviews_enabled,
            elfsight_embed_code: pageContent.elfsight_embed_code,
            is_published: true,
        ...
        console.log(`${lang}: page=${!upsertErr}, properties=${propCount}/${properties.length}`);
