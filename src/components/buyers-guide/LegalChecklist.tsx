...
const itemIcons = [FileText, Building2, CreditCard, Users, Shield];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-20 reveal-on-scroll">
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-prime-gold/10 border border-prime-gold/20 rounded-full mb-6">
            ...
            <span className="text-prime-gold text-sm font-semibold tracking-wide uppercase">{t.legal.badge}</span>
          </span>
              ...
              >
                ...
                <div className="absolute top-4 right-4 z-10">
                  <span className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide ${
                    ...
                    {item.status === 'essential' ? t.legal.essential : t.legal.optional}
                  </span>
                </div>
                ...
                <div className="relative p-6">
                  ...
                  <div className="w-16 h-16 rounded-2xl bg-prime-gold/10 flex items-center justify-center mb-5 group-hover:scale-110 group-hover:bg-prime-gold/20 transition-all duration-300">
                    <Icon className="w-8 h-8 text-prime-gold" />
                  </div>
                  ...
                  <div className="flex items-center gap-2 mb-5 p-3 bg-muted/50 rounded-xl">
                    ...
                    <span className="text-sm text-muted-foreground">{t.legal.timeline}:</span>
                    <span className="text-sm font-semibold text-prime-gold">{item.timeline}</span>
                  </div>

                  {/* Tips */}
                  <div className="border-t pt-5">
                    <p className="text-xs font-semibold text-prime-gold uppercase tracking-wide mb-3">{t.legal.proTips}</p>
              ...
              <div className="flex flex-col md:flex-row items-start gap-6 mb-10">
                ...
                <div>
                  <h3 className="text-3xl md:text-4xl font-serif font-bold text-white mb-3">
                    {t.legal.dueDiligence.title}
                  </h3>
                  <p className="text-white/70 text-lg max-w-2xl">
                    {t.legal.dueDiligence.description}
                  </p>
                </div>
              </div>

              <div className="grid md:grid-cols-2 gap-4">
                {t.legal.dueDiligence.checks.map((check, index) => (
                  <div
                    key={index}
